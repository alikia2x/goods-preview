...
Keychains use the locally bundled **Studio Small 09** HDRI by Sergej Majboroda / Poly Haven (CC0). See `public/environments/LICENSE.md` for provenance. The original 2K HDR preserves emitter luminance and dark studio surfaces; PMREM supplies roughness-dependent reflections. Its measured main-emitter direction is aligned to the light-direction control, along with the shadow-casting key light. The HDR is lighting only, never a visible background. The badge keeps its existing tuned environment profiles.
